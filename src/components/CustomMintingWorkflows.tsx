...
const TOKEN_INFO: TokenInfo[] = [
  { id: 0, name: "Token 0", description: "Free mint with 60s cooldown", isMintable: true },
  { id: 1, name: "Token 1", description: "Free mint with 60s cooldown", isMintable: true },
  { id: 2, name: "Token 2", description: "Free mint with 60s cooldown", isMintable: true },
  { id: 3, name: "Token 3", description: "Forge by burning Token 0 & 1", isMintable: false },
  { id: 4, name: "Token 4", description: "Forge by burning Token 1 & 2", isMintable: false },
  { id: 5, name: "Token 5", description: "Forge by burning Token 0 & 2", isMintable: false },
  { id: 6, name: "Token 6", description: "Forge by burning Token 0, 1 & 2", isMintable: false },
];
...
export const CustomMintingWorkflows = ({
  ...
  const [mintAmounts, setMintAmounts] = useState<{ [key: number]: number }>({ 0: 1, 1: 1, 2: 1 });
  const [burnAmounts, setBurnAmounts] = useState<{ [key: number]: number }>({ 3: 1, 4: 1, 5: 1, 6: 1 });
  const [tradeAmounts, setTradeAmounts] = useState<{ [key: number]: number }>({});
  ...
  } = useERC1155Contract({ contractAddress, isWalletConnected });
              ...
              <BrutalCardContent className="space-y-4">
                ...
                <div className="flex gap-2">
                  <BrutalInput
                    type="number"
                    min={1}
                    value={mintAmounts[token.id] || 1}
                    onChange={(e) => setMintAmounts((prev) => ({ ...prev, [token.id]: Math.max(1, parseInt(e.target.value) || 1) }))}
                    className="w-20"
                  />
                  <BrutalButton
                    onClick={() => handleMint(token.id)}
                    disabled={tradeLoading === token.id || !isWalletConnected || isCooldownActive}
                    className="flex-1"
                  >
                ...
                {!isWalletConnected && (
                  <div className="text-xs text-center text-muted-foreground">Connect wallet to mint</div>
                )}
              </BrutalCardContent>
            </BrutalCard>
          ))}
                    ...
                    <BrutalCardContent className="space-y-4">
                      ...
                      {recipe && (
                        <div className="bg-muted border-2 border-border p-3">
                          ...
                          {recipe.tokensToBurn.map((burnId) => (
                            <div key={burnId} className="flex items-center justify-between text-sm mb-1">
                      ...
                      <BrutalButton
                        ...
                        disabled={tradeLoading === token.id || !isApproved || !canMint}
                        className="w-full"
                    ...
                    <BrutalCardContent className="space-y-3">
                      <div className="flex gap-2">
                        <BrutalInput
                          ...
                          min={1}
                          max={balance}
                          value={burnAmounts[tokenId] || 1}
                          onChange={(e) => setBurnAmounts((prev) => ({ ...prev, [tokenId]: Math.max(1, Math.min(balance, parseInt(e.target.value) || 1)) }))}
                          className="w-16"
                          disabled={balance === 0}
                        />
                        <BrutalButton
                          onClick={() => handleBurnTop(tokenId)}
                          disabled={tradeLoading === tokenId || !isApproved || balance === 0}
                          className="flex-1"
                          variant="destructive"
                        >
                          ...
                          selectedTradeToken === tokenId ? (
                            <div className="space-y-2">
                              <BrutalInput
                                ...
                                min={1}
                                max={balance}
                                value={tradeAmounts[tokenId] || 1}
                                onChange={(e) => setTradeAmounts((prev) => ({ ...prev, [tokenId]: Math.max(1, Math.min(balance, parseInt(e.target.value) || 1)) }))}
                                className="w-full"
                              />
                            ...
                            <BrutalButton onClick={() => { setSelectedTradeToken(tokenId); setTradeAmounts((prev) => ({ ...prev, [tokenId]: 1 })); }} disabled={!isApproved} className="w-full" variant="secondary">
                              {!isApproved ? "Approve First" : "Trade"}
                            </BrutalButton>
                          )
                        ) : (
                          <div className="text-center text-sm text-muted-foreground p-2 bg-muted border-2 border-border">No tokens</div>
                        )}
                      </BrutalCardContent>
                    </BrutalCard>
                  );
